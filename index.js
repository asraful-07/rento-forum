const loadAllphones = async (isShowed = false, category) => {
 ...
 const displayAllPhone = (iphones) => {
    ...
    <img class="w-24 mx-auto" src=${iphone.image} alt="Product Image">
    <div class="p-5 text-center">
        <h5 class="text-xl font-semibold mb-2">${iphone.brand}</h5>
        <h5 class="text-xl font-semibold mb-2">${iphone.phone_name}</h5>
        <p class="text-gray-600 mb-4">There are many variations of passages of available, but the majority have suffered</p>
        <p class="text-2xl font-bold mb-4">${iphone.slug}</p>
        <button onclick="phoneDetails('${iphone.slug}')" class="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600">Show Details</button>
    </div>
</div>
    `;
